...
def generate_visualization(year, month, day, hour, region_coords, product):
    ...
    if product == "Dewpoint, MSLP, and 10 m wind barbs":
        ...
        with nc.Dataset(mslp_f) as ds:
            mslp = ds.variables['prmsl'][:]        # Pa
            lon0 = ds.variables['lon'][:]
        ...
        # Center to map frame & ensure increasing axes
        ...
        render_dewpoint_mslp_10m(ax, lon_plot, lat_sel, center_deg, mslp_sub, T_k_sub, RH_sub, u10_sub, v10_sub)
        ...
        # Center & ensure increasing axes
        lon_plot = to_center_frame_vec(lon_sel, center_deg)
        lon_plot, lat_sel, u_sub, v_sub, z_sub = ensure_increasing_axes(lon_plot, lat_sel, u_sub, v_sub, z_sub)

        render_pl_winds(ax, lon_plot, lat_sel, center_deg, u_sub, v_sub, z_sub, level)
